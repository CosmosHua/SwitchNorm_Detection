...
class mask_rcnn_fcn_head_v1upXconvs_sn(nn.Module):
    """v1upXconvs design: X * (conv 3x3), convT 2x2, with SwitchNorm"""
    def __init__(self, dim_in, roi_xform_func, spatial_scale, num_convs):
        super().__init__()
        self.dim_in = dim_in
        self.roi_xform = roi_xform_func
        ...
        for i in range(num_convs):
            module_list.extend([
                nn.Conv2d(dim_in, dim_inner, 3, 1, padding=1*dilation, dilation=dilation, bias=False),
                mynn.SwitchNorm(dim_inner, using_moving_average=(not cfg.TEST.USE_BATCH_AVG), 
    ...
    def detectron_weight_mapping(self):
        ...
        for i in range(self.num_convs):
            mapping_to_detectron.update({
                'conv_fcn.%d.weight' % (3*i): '_mask_fcn%d_w' % (i+1),
                'conv_fcn.%d.weight' % (3*i+1): '_mask_fcn%d_sn_s' % (i+1),
                'conv_fcn.%d.bias' % (3*i+1): '_mask_fcn%d_sn_b' % (i+1),
                'conv_fcn.%d.mean_weight' % (3*i+1): '_mask_fcn%d_sn_mean_weight' % (i+1),
                'conv_fcn.%d.var_weight' % (3*i+1): '_mask_fcn%d_sn_var_weight' % (i+1),
            })
            if cfg.MRCNN.SN.USE_BN:
                mapping_to_detectron.update({
                    'conv_fcn.%d.running_mean' % (3*i+1): '_mask_fcn%d_sn_rm' % (i+1),
                    'conv_fcn.%d.running_var' % (3*i+1): '_mask_fcn%d_sn_riv' % (i+1),
                })
        mapping_to_detectron.update({
            'upconv.weight': 'conv5_mask_w',
            'upconv.bias': 'conv5_mask_b'
